...
export default function EventHub({ allSpots = [], currentUser, userPosition = null, units = 'mi', showToast } = {}) {
  ...
  const refresh = useCallback(async () => {
    const result = await fetchUpcomingEvents(150);
      ...
      {composerOpen && (
        <form onSubmit={submit} className="surface-card mb-5 rounded-[1.6rem] border-accent-500/25 p-4">
          ...
          <div className="mt-4 space-y-3">
            <label className="block min-w-0 text-xs font-bold text-muted">Event name<input aria-label="Event name" value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} maxLength={100} required placeholder="Event name" className="surface-input mt-1.5 min-w-0 w-full rounded-2xl px-3.5 py-3 text-sm" /></label>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="block min-w-0 text-xs font-bold text-muted">Event type<select aria-label="Event type" value={form.eventType} onChange={(event) => setForm((current) => ({ ...current, eventType: event.target.value }))} className="surface-input mt-1.5 min-w-0 w-full rounded-2xl px-3.5 py-3 text-sm font-semibold"><option value="meetup">Meetup</option><option value="car_show">Car show</option><option value="cruise_in">Cruise-in</option><option value="cars_and_coffee">Cars & coffee</option></select></label>
              <label className="block min-w-0 text-xs font-bold text-muted">SnapMap spot<select aria-label="SnapMap spot" value={form.spotId} onChange={(event) => chooseSpot(event.target.value)} className="surface-input mt-1.5 min-w-0 w-full rounded-2xl px-3.5 py-3 text-sm font-semibold"><option value="">SnapMap spot (optional)</option>{cloudSpots.map((spot) => <option key={spot.id} value={spot.id}>{spot.name}</option>)}</select></label>
            </div>

            <div className="grid gap-3 sm:grid-cols-2">
              <label className="block min-w-0 text-xs font-bold text-muted">Venue name<input aria-label="Venue name" value={form.venueName} onChange={(event) => setForm((current) => ({ ...current, venueName: event.target.value }))} maxLength={160} placeholder="Venue name (optional)" className="surface-input mt-1.5 min-w-0 w-full rounded-2xl px-3.5 py-3 text-sm" /></label>
              <label className="block min-w-0 text-xs font-bold text-muted">Address<input aria-label="Address" value={form.address} onChange={(event) => setForm((current) => ({ ...current, address: event.target.value }))} maxLength={300} placeholder="Address (optional)" className="surface-input mt-1.5 min-w-0 w-full rounded-2xl px-3.5 py-3 text-sm" /></label>
            </div>

            <div className="grid gap-3 sm:grid-cols-2"><label className="block text-xs font-bold text-muted">Starts<input type="datetime-local" value={form.startsAt} onChange={(event) => setForm((current) => ({ ...current, startsAt: event.target.value }))} required className="surface-input mt-1.5 w-full rounded-2xl px-3.5 py-3 text-sm text-primary" /></label><label className="block text-xs font-bold text-muted">Ends <span className="font-normal">(optional)</span><input type="datetime-local" value={form.endsAt} onChange={(event) => setForm((current) => ({ ...current, endsAt: event.target.value }))} className="surface-input mt-1.5 w-full rounded-2xl px-3.5 py-3 text-sm text-primary" /></label></div>
            <label className="block min-w-0 text-xs font-bold text-muted">Event details<textarea aria-label="Event details" value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} maxLength={1200} rows={3} placeholder="What’s the plan? Add arrival details, what to bring, or the kind of shots you’re after." className="surface-input mt-1.5 min-w-0 w-full resize-none rounded-2xl px-3.5 py-3 text-sm" /></label>
            <details className="rounded-2xl border border-[var(--border-subtle)] p-3" onInvalidCapture={(event) => { event.currentTarget.open = true; }}>
              <summary className="cursor-pointer text-sm font-bold text-secondary">Attendance & repeat options</summary>
              ...
              <article key={event.id} className="surface-card overflow-hidden rounded-[1.6rem]">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3 border-t border-white/[0.06] pt-3">
                    {event.listingType === 'listed' ? <span className="truncate text-[11px] font-bold text-secondary">Listed from {event.sourceLabel}{event.lastVerifiedAt ? ` · ${verifiedLabel(event.lastVerifiedAt)}` : ''}</span> : <Link to={event.host?.username ? `/user/${event.host.username}` : '/explore?view=community'} className="flex min-w-0 items-center gap-2"><span className="h-7 w-7 shrink-0 overflow-hidden rounded-full bg-accent-500/15">{event.host?.avatar_url && <img src={event.host.avatar_url} alt="" className="h-full w-full object-cover" />}</span><span className="truncate text-[11px] font-bold text-secondary">{event.host?.display_name || 'Creator'}</span></Link>}
